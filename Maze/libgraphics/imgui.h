 ...
 *
 *		void MouseEventProcess(int i, int j, int button, int event)
 *		{
 *			uiGetMouse(i,j,button,event);
 *			...其他代码...
 *		}
 *
 ...
 * 功能：显示一个按钮（button）
 *
 * 用法：if( button(GenUUID(0),i,j,w,h,label) ) {
 *           执行语句，响应用户按下该按钮
 *       }
 *
 *   id  - 唯一号
 ...
 * 功能：显示一组菜单
 *
 * 用法：choice = menuList(GenUUID(0),i,j,w,h,labels,n);
 *
 *   id  - 菜单的唯一号
 *   i,j - 菜单的位置
 *   w,h - 菜单项的大小
 ...
 * 应用举例：
	...
	selection = menuList(GenUIID(0), i, j, w, wlist, h, menuListFile, sizeof(menuListFile)/sizeof(menuListFile[0]));
 ...
 *
 * 用法：textbox(GenUUID(0),i,j,w,h,textbuf,buflen);
 *       或者
         if( textbox(GenUUID(0),i,j,w,h,textbuf,buflen) ) {
 *           文本字符串被编辑修改了，执行相应语句
 *       }
 *
