 ...
 *  DefineColor("Light Gray", 1, .97, .82);
 ...
 * 功能：显示一个组菜单
 *
 * 用法：choice = menuList(GenUIID(0),x,y,w,h,labels,n);
 *
 *   id  - 菜单的唯一号
 *   x,y,w,h - 菜单类别的位置和大小
 *   wlist,h - 菜单列表的宽度和高度
 ...
 *
 * 用法：textbox(GenUIID(0),x,y,w,h,textbuf,buflen);
 *       或者
         if( textbox(GenUIID(0),x,y,w,h,textbuf,buflen) ) {
 *           文本字符串被编辑修改了，执行相应语句
 *       }
 *
 *   id  - 唯一号，一般用GenUIID(0), 或用GenUIID（k)（k是循环变量）
 *   x,y - 文本框位置
 *   w,h - 文本框的宽度和高度
...
int textbox(int id, double x, double y, double w, double h, char textbuf[], int buflen)
{
	...
	if ( gs_UIState.kbdItem == id )
	{
		...
		case VK_BACK:
			if( len > 0 ) {
				if (textbuf[len-1] < 0) --len; //汉字占2个字节且每个字节最高位都为1 by xdq @20210517
